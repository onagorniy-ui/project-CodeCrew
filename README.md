...
- Вгадувати команду, якщо користувач зробив невелику помилку в написанні (інтелектуальна автопідказка).
- Швидко повертати та повторювати попередні команди за допомогою стрілочок `↑` та `↓` у терміналі (історія команд).
...
- `add-tag [назва] #[тег]` — додати тег до нотатки (наприклад: `add-tag Python #backend`).
- `find-note [текст]` — знайти нотатки за ключовим словом або фрагментом тексту.
- `#[тег]`, `find-tag [тег]` — швидкий пошук нотаток за тегом прямо через хештег (наприклад: `#dev` або `#python`).
