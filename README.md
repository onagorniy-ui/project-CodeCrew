...
NORA — персональний консольний помічник рекрутера для зручної організації контактів кандидатів та нотаток після комунікації і співбесід. Проєкт розроблено як фінальну командну роботу.
...
- `all` — показати всіх кандидатів з усіма даними.
...
- `find-note [текст]` — знайти нотатки за ключовими словами.
- `find-tag [тег]` — знайти нотатки за тегом.
- `sort-notes` — вивести всі нотатки, посортовані за тегами.
- `all-notes` — показати всі збережені нотатки.
- `delete-note [назва]` — видалити нотатку.
- `close`, `exit` — завершити роботу та зберегти всі дані.
